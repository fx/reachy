...
#   upstream-path:    tests/unit/test_unit.py
...
class TestGetVersion:
    def setup_method(self):
        ...
        import reachy_mini_ha_satellite.esphome.util as util
...
class TestGetEsphomeVersion:
    ...
    def test_returns_version_when_package_installed(self):
        ...
        with patch("reachy_mini_ha_satellite.esphome.util.version", return_value="42.7.0"):
    ...
    def test_returns_unknown_when_package_not_installed(self):
        ...
        with patch("reachy_mini_ha_satellite.esphome.util.version", side_effect=PackageNotFoundError):
            result = util.get_esphome_version()
        assert result == "unknown"
    ...
    def test_caches_result_after_first_call(self):
        ...
        with patch("reachy_mini_ha_satellite.esphome.util.version", return_value="1.0.0") as mock_ver:
    ...
    def test_returns_cached_value_on_second_call(self):
        ...
        with patch("reachy_mini_ha_satellite.esphome.util.version", return_value="5.0.0"):
...
class TestCallAll:
    def test_calls_single_callable(self):
        from reachy_mini_ha_satellite.esphome.util import call_all
...
class TestGetDefaultInterface:
    def test_returns_interface_name_from_gateway(self):
        ...
        with patch("reachy_mini_ha_satellite.esphome.util.netifaces") as mock_netifaces:
            # Set AF_INET before building the dict so the key matches
            mock_netifaces.AF_INET = 2
            mock_netifaces.default_gateway.return_value = {2: ("192.0.2.1", "eth0")}
    ...
    def test_returns_none_when_no_ipv4_gateway(self):
        ...
        with patch("reachy_mini_ha_satellite.esphome.util.netifaces") as mock_netifaces:
            ...
            mock_netifaces.default_gateway.return_value = {99: ("198.51.100.1", "eth1")}
            result = util.get_default_interface()
        assert result is None
...
class TestGetDefaultIpv4:
    def test_returns_ip_for_interface(self):
        ...
        with patch("reachy_mini_ha_satellite.esphome.util.netifaces") as mock_netifaces:
            mock_netifaces.AF_INET = 2
            mock_netifaces.ifaddresses.return_value = {2: [{"addr": "192.0.2.50"}]}
