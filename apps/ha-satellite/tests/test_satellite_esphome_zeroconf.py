...
#   upstream-path:    tests/unit/test_zeroconf.py
...
def make_zeroconf(**kwargs):
    defaults = dict(
        ...
        host_ip_address="192.0.2.100",
        name="lva-test",
    )
    defaults.update(kwargs)

    with patch("reachy_mini_ha_satellite.esphome.zeroconf.AsyncZeroconf") as mock_zc_cls:
        mock_zc = MagicMock()
...
class TestInit:
    ...
    def test_host_ip_stored(self):
        zc = make_zeroconf(host_ip_address="198.51.100.1")
        assert zc.host_ip_address == "198.51.100.1"

    def test_name_defaults_to_mac_when_not_provided(self):
        with patch("reachy_mini_ha_satellite.esphome.zeroconf.AsyncZeroconf"):
            from reachy_mini_ha_satellite.esphome.zeroconf import HomeAssistantZeroconf
    ...
    def test_async_zeroconf_instantiated(self):
        with patch("reachy_mini_ha_satellite.esphome.zeroconf.AsyncZeroconf") as mock_cls:
...
class TestRegisterServer:
    ...
    async def test_register_service_called(self):
        ...
        with patch("reachy_mini_ha_satellite.esphome.zeroconf.AsyncServiceInfo"):
    ...
    async def test_service_name_contains_device_name(self):
        ...
        with patch("reachy_mini_ha_satellite.esphome.zeroconf.AsyncServiceInfo") as mock_info_cls:
    ...
    async def test_service_address_matches_host_ip(self):
        ...
        zc = make_zeroconf(host_ip_address="198.51.100.5")
        zc._mock_zc.async_register_service = AsyncMock()
        ...
        assert socket.inet_aton("198.51.100.5") in addresses
