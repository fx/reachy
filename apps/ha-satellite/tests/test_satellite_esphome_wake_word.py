...
#   upstream-path:    tests/unit/test_wake_word.py
...
class TestFindAvailableWakeWords:
    def test_finds_micro_wake_word(self, tmp_path):
        ...
        from reachy_mini_ha_satellite.esphome.wake_word import find_available_wake_words
...
class TestLoadWakeModels:
    ...
    def test_loads_requested_wake_word(self, tmp_path):
        ...
        from reachy_mini_ha_satellite.esphome.wake_word import load_wake_models
...
class TestLoadStopModel:
    def test_returns_model_when_found(self, tmp_path):
        write_json(tmp_path / "stop.json", make_micro_json("stop"))
        mock_model = MagicMock()
        with patch("reachy_mini_ha_satellite.esphome.wake_word.MicroWakeWord.from_config", return_value=mock_model):
            from reachy_mini_ha_satellite.esphome.wake_word import load_stop_model
    ...
    def test_returns_none_when_load_fails(self, tmp_path):
        write_json(tmp_path / "stop.json", make_micro_json("stop"))
        with patch("reachy_mini_ha_satellite.esphome.wake_word.MicroWakeWord.from_config", side_effect=Exception("load error")):
            from reachy_mini_ha_satellite.esphome.wake_word import load_stop_model
    ...
    def test_stops_at_first_found(self, tmp_path):
        ...
        write_json(dir2 / "stop.json", make_micro_json("stop"))
        mock_model = MagicMock()
        with patch("reachy_mini_ha_satellite.esphome.wake_word.MicroWakeWord.from_config", return_value=mock_model) as mock_load:
            from reachy_mini_ha_satellite.esphome.wake_word import load_stop_model
