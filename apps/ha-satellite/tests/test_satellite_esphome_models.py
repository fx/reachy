...
#   upstream-path:    tests/unit/test_models.py
...
from esphome_test_support import make_state as make_server_state


def make_preferences(**kwargs):
    from reachy_mini_ha_satellite.esphome.models import Preferences
...
class TestInitialStopWordThreshold:
    def test_saved_sensitivity_is_restored(self):
        from reachy_mini_ha_satellite.esphome.models import initial_stop_word_threshold

        assert initial_stop_word_threshold(0.9) == pytest.approx(0.9)

    def test_falls_back_to_server_state_default_when_never_set(self):
        from reachy_mini_ha_satellite.esphome.models import ServerState, initial_stop_word_threshold
